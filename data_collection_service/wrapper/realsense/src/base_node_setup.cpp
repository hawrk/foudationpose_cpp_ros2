...
void BaseRealSenseNode::startPublishers(const std::vector<stream_profile>& profiles, const RosSensor& sensor)
{
    ...
    for (auto& profile : profiles)
    {
        ...
        _metadata_publishers[sip] = _node.create_publisher<dros_common_interfaces::msg::Metadata>(topic_metadata, 
            rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(info_qos), info_qos));
        ...
        if (!((rs2::stream_profile)profile==(rs2::stream_profile)_base_profile))
        {
            ...
            _extrinsics_publishers[sip] = _node.create_publisher<dros_common_interfaces::msg::Extrinsics>(topic_extrinsics,
...
void BaseRealSenseNode::startRGBDPublisherIfNeeded()
{
    ...
    if(_enable_rgbd && !_rgbd_publisher)
    {
        ...
        {
            ROS_INFO_STREAM("startRGBDPublisher");
            rmw_qos_profile_t qos = _use_intra_process ? qos_string_to_qos(DEFAULT_QOS) : qos_string_to_qos(IMAGE_QOS);

            // adding "~/" to the topic name will add node namespace and node name to the topic
            // see "Private Namespace Substitution Character" section on https://design.ros2.org/articles/topic_and_service_names.html
            _rgbd_publisher = _node.create_publisher<dros_common_interfaces::msg::RGBD>("rgbd",
                rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(qos), qos));
        }
        else {
...
void BaseRealSenseNode::publishServices()
{
    ...
    // see "Private Namespace Substitution Character" section on https://design.ros2.org/articles/topic_and_service_names.html
    _device_info_srv = _node.create_service<dros_common_interfaces::srv::DeviceInfo>(
            "~/device_info",
            [&](const dros_common_interfaces::srv::DeviceInfo::Request::SharedPtr req,
                        dros_common_interfaces::srv::DeviceInfo::Response::SharedPtr res)
                        {getDeviceInfo(req, res);});
}

void BaseRealSenseNode::getDeviceInfo(const dros_common_interfaces::srv::DeviceInfo::Request::SharedPtr,
                                            dros_common_interfaces::srv::DeviceInfo::Response::SharedPtr res)
{
